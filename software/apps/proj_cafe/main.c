...
int BLE_ADV_TIMEOUT = ADV_INTERVAL * 7.5 / 10; // units of 10 ms
...
static simple_ble_config_t ble_config = {
    ...
    .device_id = 0xCAFE, // used as the 5th and 6th octet in the device BLE address, you will need to change this for each device you have
    .adv_name = "CAFE",
...
void broadcast_next_message() {
    if(mesh_assemble_next_message(ble_data) == 0) {
        DEBUG_PRINT("about to send reg %d\n", ble_data[REG_NUM_OFFSET]);
        advertising_stop();
