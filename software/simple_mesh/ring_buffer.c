...
inline __attribute__((always_inline)) void q_push(queue_t* q, uint8_t reg_num)
{
    ...
    DEBUG_PRINT("pushing %d to queue\n", reg_num);
...
inline __attribute__((always_inline)) uint8_t q_pop(queue_t* q)
{
    ...
    printf("dumb dumb, you popped an empty boi");
    APP_ERROR_HANDLER(123);
    return -1;
}
...
inline __attribute__((always_inline)) void q_print_status(queue_t* q)
{
    DEBUG_PRINT("In queue: ");
    for (uint16_t i = 0; i < 256; i++) {
        if ((q->data[i >> 3] & (1 << ((i & 0b111) - 1))) != 0) {
            DEBUG_PRINT("%d, ", i);
        }
    }
    DEBUG_PRINT("\n");
